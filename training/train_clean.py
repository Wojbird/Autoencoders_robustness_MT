...
def train_clean_model(
    ...
    *,
    ...
    noise_std = float(cfg.get("eval_noise_std", cfg.get("noise_std", 0.0)))
    ...
    if log_wandb:
        run = wandb.init(
            ...
            config={**cfg, "dataset_type": dataset_type, "training_variant": "clean", "model_class": model_class.__name__, "gpu_id": gpu_id},
        )
    ...
    try:
        for epoch in range(1, epochs + 1):
            ...
            if log_wandb:
                wandb.log({"val/reconstructions": wandb.Image(image_path, caption=f"{model_name} | epoch {epoch}")})
