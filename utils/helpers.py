        ...
        if gpu_id is not None:
            if gpu_id < 0 or gpu_id >= torch.cuda.device_count():
                raise ValueError(
                    ...
                    f"Available GPU ids: 0..{torch.cuda.device_count() - 1}"
                )
...
def save_images(
    ...
    with torch.no_grad():
        for x, _ in dataloader:
            ...
            out_tensor = out_tensor.clamp(0.0, 1.0).cpu()
            x_vis = x_vis.cpu()
    ...
    if images_shown == 0:
        raise ValueError("No images available for save_images().")
...
def ensure_val_fraction(val_set, fraction: float, split_seed: int = 42):
    ...
    if fraction <= 0.0:
        raise ValueError("val_subset_fraction must be > 0.0.")
...
def init_csv_logger(csv_path: str, extra_columns=None, overwrite: bool = True):
    ...
    new_file = overwrite or not os.path.exists(csv_path)
...
def safe_save_state_dict(state_dict, final_ckpt_path: str, retries: int = 3, delay: float = 2.0):
    ...
    final_tmp_path = final_ckpt_path + f".pid{os.getpid()}.tmp"
