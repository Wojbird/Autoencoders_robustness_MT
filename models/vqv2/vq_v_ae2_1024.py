...
class VQVAE21024(nn.Module):
    def __init__(self, config):
        ...
        self.enc_b3 = nn.Sequential(
            nn.Conv2d(256, self.bottom_dim, kernel_size=3, stride=2, padding=1),
            nn.InstanceNorm2d(self.bottom_dim),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Dropout2d(0.2)
        )

        self.enc_t1 = nn.Sequential(
            nn.Conv2d(self.bottom_dim, 512, kernel_size=3, stride=2, padding=1),
            nn.InstanceNorm2d(512),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Dropout2d(0.2)
        )
        self.enc_t2 = nn.Sequential(
            nn.Conv2d(512, self.top_dim, kernel_size=3, stride=1, padding=1),
            nn.InstanceNorm2d(self.top_dim),
            nn.LeakyReLU(0.1, inplace=True),
        )
        ...
        self.dec1 = nn.Sequential(
            nn.ConvTranspose2d(self.bottom_dim * 2, 256, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.InstanceNorm2d(256),
            nn.LeakyReLU(0.1, inplace=True),
        )
...
config_path = "configs/vqv2/vq_v_ae2_1024.json"
